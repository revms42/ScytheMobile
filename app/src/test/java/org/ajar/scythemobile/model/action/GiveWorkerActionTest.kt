...
class GiveWorkerActionTest {
    ...
    fun testGiveWorkerAction() {
        assertTrue(ScytheAction.GiveWorkerAction(1, playerInstance, 1).perform())

        val unit: GameUnit? = playerInstance.selectUnits(UnitType.WORKER)?.firstOrNull { it.pos == 1 }?.also { workers.add(it.unitData) }
        assertNotNull(unit)
